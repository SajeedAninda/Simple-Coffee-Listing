...
const Card = ({ showAvailable }) => {
  ...
  const filteredItems = showAvailable
    ? coffeeItems.filter((coffee) => coffee.available)
    : coffeeItems;
      ...
      {filteredItems.map((coffee) => (
        <div key={coffee.id}>
          ...
          <div className="flex justify-between items-center mt-2">
            <p className="text-white font-bold text-[16px]">{coffee.name}</p>
          ...
          <div className="flex justify-between items-center  mt-2">
            <div className="flex gap-2">
              ...
              <div className="flex gap-1">
                {coffee.rating === null ? (
                  <p className="text-[#6F757C] font-bold text-[16px]">No Rating</p>
                ) : (
                  <p className="text-white font-bold text-[16px]">{coffee.rating}</p>
                )}
            ...
            {!coffee.available && (
              <p className="text-[#ED735D] font-bold text-[16px]">Sold Out</p>
            )}
          </div>
        </div>
      ))}
